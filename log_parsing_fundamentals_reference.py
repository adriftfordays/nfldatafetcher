...
# ❌ WRONG: log_info = {"date" = parts[0]}     # Using = instead of :
# ❌ WRONG: log_info = {date: parts[0]}        # Missing quotes on key
# ✅ RIGHT: log_info = {"date": parts[0]}      # Colon for assignment, quotes for string keys

# DICTIONARY ACCESS - Common errors:  
# ❌ WRONG: log_info.date                      # Using dot notation (wrong language)
# ❌ WRONG: log_info[date]                     # Missing quotes
# ✅ RIGHT: log_info["date"]                   # Square brackets with quoted key

# SPLIT WITH LIMIT - Common errors:
# ❌ WRONG: parts = log_line.split("", 4)      # Empty string delimiter  
# ❌ WRONG: parts = log_line.split(" ", "4")   # Number as string
# ✅ RIGHT: parts = log_line.split(" ", 4)     # Space delimiter, integer limit
